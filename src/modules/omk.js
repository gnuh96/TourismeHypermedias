export class omk {
  constructor(params) {
    ...
    this.vocabs = params.vocabs
      ? params.vocabs
      : ['dcterms', 'foaf', 'skos', 'jdc', 'fup8']
    this.user = false
    this.props = []
    this.class = []
    this.rts
    let perPage = 1000,
      types = {items: 'o:item', media: 'o:media'}
    ...
    this.getPropTerm = function (id) {
      console.log(
        '1',
        me.props.filter(prp => prp['o:id'] == id),
      )
      return me.props.filter(prp => prp['o:id'] == id)[0]['o:term']
    }
    this.getClass = function (prefix, cb = false) {
      let url =
          me.api + 'resource_classes?per_page=1000&vocabulary_prefix=' + prefix,
        data = syncRequest(url)
    ...
    this.getClassByName = function (cl) {
      let c = me.class.filter(
        c => c['o:label'].toLowerCase() == cl.toLowerCase(),
      )
      return c[0]
    }
    ...
    this.getRandomItemByClass = function (cl, cb = false) {
      ...
      try {
        url =
          me.api + 'items?resource_class_id=' + me.getClassByName(cl)['o:id']
      } catch (error) {
        console.error(error)
      }
    ...
    this.getAllMedias = function (query, cb = false) {
      let url = me.api + 'media?per_page=' + perPage + '&' + query + '&page=',
        fin = false,
        rs = [],
        data,
        page = 1
    ...
    this.createRessource = function (data, cb = false, type = 'items') {
      let url =
        ...
        '&key_credential=' +
        me.key
      postData({u: url, m: 'POST'}, me.formatData(data), data['file']).then(
        rs => {
          if (cb) cb(rs)
        },
      )
    ...
    this.updateRessource = function (
      ...
      let url =
        ...
        '&key_credential=' +
        me.key
      postData({u: url, m: m}, fd ? fd : me.formatData(data, types[type])).then(
        rs => {
          if (cb) cb(rs)
        },
      )
      ...
      for (let [k, v] of Object.entries(data)) {
        switch (k) {
          ...
          default:
            ...
            } else fd[k].push(formatValue(p, v))
            break
        }
      }
      return fd
